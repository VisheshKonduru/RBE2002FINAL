...
protected: 
    ...
    */
    ...
    {
        static float previousError = 0.0;  // Store the previous error for derivative calculation
        static float integral = 0.0;       // Store the cumulative integral of the error
        float error = targetSpeed - speed; // Calculate the current error
        const float integralCap = 80.0;
        ...
        if(ctrlMode == CTRL_SPEED)
        {
            ...
            int16_t effort = (Kp * error) + (Ki * integral) + Kd * (error - previousError) ;
